...
* **set** - `{a, b, c}` - Modules `a`, `b`, `c` are run in parallel. All three receive the same event from their parent module.
...
A workflow can emit outputs to an attached module. The outputs from a `set` or `dict` workflow will be batched and emitted one by one to the attached module. The output from the last module in a `list` workflow will be emitted to the attached module.
