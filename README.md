...
>
> After that, there is no limitation on the number of arguments that can be sent to an event handler. Please ensure that the signature of the event handler functions matches the arguments being sent to it via `dispatch_event()` or `emit_event()`.
>
